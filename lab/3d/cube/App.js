...
export default class App {
  constructor() {
    ...
    this.pixi = new PIXI.Application(width, height, {resizeTo: window.parent.getParent(), backgroundColor: 0x191919});
  ...
  initializeGUI() {
    ...
    settings.autorotate = this.autorotate.bind(this);
    settings.reset = this.reset.bind(this);
    gui.add(settings, 'x').min(0).max(this.canvas.width).step(1).onChange((value) => {
      var dx = value - settings.prevx;
      settings.prevx = value;
      this.cube.setProperty('x', dx);
    });
    gui.add(settings, 'y').min(0).max(this.canvas.height).step(1).onChange((value) => {
      var dy = value - settings.prevy;
      settings.prevy = value;
      this.cube.setProperty('y', dy);
    });
    gui.add(settings, 'z').min(-300).max(300).step(1).onChange((value) => {
      var dz = value - settings.prevz;
      settings.prevz = value;
